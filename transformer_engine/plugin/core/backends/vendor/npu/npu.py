...
class NPUBackend(TEFLBackendBase):
    ...
    def layernorm_fwd(
        ...
        """LayerNorm forward using torch_npu.npu_layer_norm.

        TE-FL calls with:
          (input, weight, bias, eps, ln_out, quantizer, otype, sm_margin, zero_centered_gamma)
        NPU kernel:
          npu_layer_norm(input, normalized_shape, weight, bias, eps) → (output, mean, rstd)

        NPU kernel requires 2D input [outer_dim, inner_dim]. We reshape accordingly.
        We ignore ln_out (pre-allocated output buffer), otype, sm_margin.

        NPU supported combo (BF16):
          input(BF16) weight(BF16) bias(BF16) → output(BF16) mean(FP32) rstd(FP32)
        """

        if zero_centered_gamma:
            weight = weight + 1

        # NPU npu_layer_norm requires 2D input: [outer_dim, hidden_size]
        input_shape = input.shape
        inner_dim = weight.shape[0]
        x_2d = input.reshape(-1, inner_dim)

        # Handle bias (use zeros if None)
        if bias is None:
            bias = torch.zeros_like(weight)

        # Call NPU kernel
        # npu_layer_norm(input, normalized_shape, weight, bias, eps)
        out_2d, mean, rstd = _get_torch_npu().npu_layer_norm(x_2d, [inner_dim], weight, bias, eps)
        ...
        # TE-FL expects (ln_out, mu, rsigma)
        # mean shape is [outer_dim] from NPU kernel
        # rstd shape is [outer_dim] from NPU kernel
    ...
    def layernorm_bwd(
        ...
        """LayerNorm backward using torch_npu.npu_layer_norm_backward.

        TE-FL calls with: (dz, x, mu, rsigma, gamma, sm_margin, zero_centered_gamma)
        NPU kernel expects:
          npu_layer_norm_backward(grad_out, input, normalized_shape, mean, rstd, weight, bias)

        NPU kernel requires 2D input [outer_dim, inner_dim].

        NPU supported combo (BF16):
          grad_out(BF16) input(BF16) mean(FP32) rstd(FP32) weight(BF16)
          → grad_input(BF16) grad_weight(FP32) grad_bias(FP32)
        """
        ...
        # NPU kernel requires mean and rstd in float32
        mean_fp32 = mu.float() if mu.dtype != torch.float32 else mu
        rstd_fp32 = rsigma.float() if rsigma.dtype != torch.float32 else rsigma

        # Call NPU kernel
        # npu_layer_norm_backward(grad_out, input, normalized_shape, mean, rstd, weight, bias)
        dx_2d, dgamma, dbeta = _get_torch_npu().npu_layer_norm_backward(
            dz_2d, x_2d, [inner_dim], mean_fp32, rstd_fp32, gamma, None
        )
        ...
        # Return gradients
        # Note: dgamma and dbeta are already in the correct shape from NPU kernel
